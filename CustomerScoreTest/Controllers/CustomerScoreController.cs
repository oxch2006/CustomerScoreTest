...
namespace CustomerScoreTest.Controllers
{
    ...
    public class CustomerScoreController : ControllerBase
    {
        ...
        {
            if (score < -1000 || score > 1000) throw new Exception("score should be in range of -1000 and 1000");
            var restult =await _customerScoreService.UpdateCustomer(customerid, score);
            return Ok(restult);
        }

        [ProducesResponseType(typeof(GetGustomersByRankResponse), 200)]
        [HttpGet("~/leaderboard")]
        public async Task<IActionResult> GetGustomersByRank([FromQuery]int start, [FromQuery]int end)
        {
            var customers = await _customerScoreService.GetGustomersByRank(start, end);
            var result = new GetGustomersByRankResponse()
            {
                Customers = customers
            };
            return Ok(result);
        ...
        [ProducesResponseType(typeof(GetGustomersByCustomerIdResponse), 200)]
        ...
        {
            var customers = await _customerScoreService.GetGustomersByCustomerId(customerid, high, low);
            var result = new GetGustomersByCustomerIdResponse()
            {
                Customers = customers
            };
            return Ok(result);
